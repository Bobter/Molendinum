...
public class GameManager : MonoBehaviour
{
    ...
    public int[] availableTokens = { 0, 0 };//contador de la cantidad de fichas activas
    ...
    Token[,] arrayToken;
    CheckboxStatus selectedCheckbox;
    Selector selector;
    Token selectedToken;
    ...
    void Start()
    {
        availableTokens =new int[] { maxTokens, maxTokens };
    ...
    void Update()
    {
        ...
        {
            if (winTextUI != null) {//si existe el texxto de victoria
                if (currentPlayerIndex == 0) winTextUI.color = Color.black;//si la victoria es del jugador 1 entonces el color del texto es negro, 
                else winTextUI.color = Color.white;//si es del jugador 2 entonces el texto es blanco
        ...
        }else if(Input.GetKeyDown(KeyCode.Mouse0)&&!movingToken)//si no hay victoria y se preciona el click izquierdo del mouse
        {
            ...
            else
            {
                if (placedTokens[currentPlayerIndex] < maxTokens)
                    PlaceToken();//si no se hace un molino entonces los jugadores deberán seguir colocando sus fichas en el tablero
                else
                    MoveToken();//si no se hace un molino entonces los jugadores deberán seguir  moviendo sus fichas por las casillas
            }           
        }
    }
    ...
    {
        GameObject select = selector.SelectObject(currentPlayerIndex);//se guarda el objeto seleccionado
